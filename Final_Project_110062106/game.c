...
ALLEGRO_TIMER* power_up_timer;  // add by Andrew
ALLEGRO_TIMER* pre_timer;
ALLEGRO_TIMER* spark_timer;
bool g_bPower_Time_Up = false; 
bool g_bSpark_Time_Up = true;
bool g_bpre_Time_Up = false;
...
static void allegro5_init(void) {
	...
	power_up_timer= al_create_timer(10); // add by andrew
	if (!power_up_timer)
		game_abort("failed to create power timer");

	spark_timer = al_create_timer(6); // add by andrew
	if (!spark_timer)
		game_abort("failed to create spark timer");

	pre_timer = al_create_timer(4); // add by andrew
	if (!pre_timer)
		game_abort("failed to create pre timer");
	...
	al_register_event_source(game_event_queue, al_get_timer_event_source(spark_timer));
	al_register_event_source(game_event_queue, al_get_timer_event_source(pre_timer));
...
static void game_start_event_loop(void) {
	...
	while (!gameDone) {
		...
		if (event.type == ALLEGRO_EVENT_DISPLAY_CLOSE) {
			...
			else if (event.timer.source == pre_timer)
			{
				game_log("pre time up");
				al_stop_timer(pre_timer);
				g_bpre_Time_Up = false;
				g_bSpark_Time_Up = false;
				al_start_timer(spark_timer);
			}
			else if (event.timer.source == spark_timer)
			{
				game_log("spark time up");
				al_stop_timer(spark_timer);
				g_bSpark_Time_Up = true;
			}
			else if (event.timer.source == power_up_timer)
			{
				game_log("Power up timer time up");
				g_bPower_Time_Up = true;
