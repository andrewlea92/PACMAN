...
static bool pacman_movable(Pacman* pacman, Map* M, Directions targetDirec) {
	...
	int x_pac = pacman->objData.Coord.x;
	int y_pac = pacman->objData.Coord.y;
	...
	if (is_wall_block(M, x_pac, y_pac) || is_room_block(M, x_pac, y_pac))
		return false;
...
void pacman_draw(Pacman* pman) {
	...
	*/
	...
	//Draw default image
	//game_log(pman->objData.moveCD);
	al_draw_scaled_bitmap(pman->move_sprite, nBase  + (pman->objData.moveCD / 32) * 16, 0,
		16, 16,
		drawArea.x + fix_draw_pixel_offset_x, drawArea.y + fix_draw_pixel_offset_y,
		draw_region, draw_region, 0
	);
