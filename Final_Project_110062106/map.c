...
int map_offset_x = 25, map_offset_y = 50;			// pixel offset of where to start draw map
...
static void draw_earthquake_bean(Map* M, const int row, const int col);

extern bool cheat_mode;
bool g_bEarthQuake = false;
int g_nEarthQuakeCount = 0;
...
void draw_map(Map const* M) {
	...
	if (g_bEarthQuake)
	{
		map_offset_x = 25 + ((rand() % 10) - 5);
		map_offset_y = 50 + ((rand() % 10) - 5);
	}
	else
	{
		map_offset_x = 25;
		...
		for (int col = 0; col < M->col_num; col++) {
			switch (M->map[row][col])
			{
				...
				case 'P':
					...
					break;
				case 'E':
					draw_earthquake_bean(M, row, col);
					break;
				case '.':
					draw_bean(M, row, col);
					break;
